...
$theme_name = 'klasseztheme';

define('THEME_NAME', $theme_name);
define('ET_DOMAIN', 'localhost');
...
class ET_FreelanceEngine extends AE_Base
{
    function __construct() {
        ...
        /**
         * register wp_role TUTOR
         */
        if (!isset($wp_roles->roles[TUTOR])) {

            //all new roles
            add_role(TUTOR, __('Tutor', ET_DOMAIN) , array(
                'read' => true,
         ...
         */
        if (!isset($wp_roles->roles[STUDENT])) {
            add_role(STUDENT, __('Student', ET_DOMAIN) , array(
                'read' => true,
    ...
    function fre_init() {
        ...
        /**
         * when student close project or tutor quit a project, it change to disputing
         */
        ...
        /**
         * when a user dont want student hide/contact him,
    ...
    function add_custom_role(){
        $role = array( TUTOR => TUTOR, STUDENT=>STUDENT);
        return $role;
    }
    function add_user_default_values($result) {
        if (ae_user_role($result) == TUTOR) {
    ...
    public function sync_profile_data($result) {
        ...
        if (ae_user_role($result) == TUTOR && !empty($profile) && $profile->post_type == "profile") {
    ...
    function restrict_pages() {
        ...
        //     }
        // }
        // prevent tutor post project
        if (is_page_template('page-submit-project.php')) {
            if (!fre_share_role() && ae_user_role() == TUTOR) {
                wp_redirect(home_url());
                exit;
            }
        }
    ...
    */
    function admin_comment_types_dropdown($comment_types) {
        $comment_types['fre_review']   = __("Tutor Review", ET_DOMAIN);
        $comment_types['em_review']   = __("Student Review", ET_DOMAIN);
    ...
    function filter_link_redirect_register($result) {
        ...
        $redirect_url = ($role == "student" && AE_Users::is_activate($result->ID) ) ? et_get_page_link('submit-project') : et_get_page_link('profile');
    ...
    function ae_check_role_user($user_data) {
        if ( isset($user_data['role'] ) && ($user_data['role'] != TUTOR && $user_data['role'] != STUDENT)) {
    ...
    */
    ...
    function ae_required_plugins() {
        ...
        $config = array(
            ...
            'default_path' => '',
            'student_menu_slug' => 'themes.php',
            'student_url_slug' => 'themes.php',
...
add_action( 'wp_ajax_nopriv_switch_profile', 'update_roles' );
add_action( 'wp_ajax_switch_profile', 'update_roles' );
// switch user role ANEES EDITING
function update_roles()
{
   ...
   {
       // Remove role
       $current_user->remove_role( 'tutor');
  ...
  {
    // Remove role
    $current_user->remove_role( 'student');

    // Add role
    $current_user->add_role( 'tutor');
      $result['new'] = 'tutor';
  }
